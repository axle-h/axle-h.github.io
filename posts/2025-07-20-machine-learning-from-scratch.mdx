...
* I give the model the delta of each feature **AND** the absolute values so it has the context of the current action plus the global state of the game.
...
* 2 fully connected hidden layers with 10 neurons each
...
1. Applies a weighted sum over input values.
2. Adds a constant value to the result, so that an activation bias is possible.
3. Passes the result through an activation function, which introduces non-linearity into the model.
...
There was definitely a breakthrough at around generation 6,000 that allowed the model to start scoring in the hundreds of millions.
This is amazing but demonstrates the drawbacks of using unsupervised learning for a problem as complex as Tetris.
...
TODO record the 647931149 score run, maybe exclusively in GB mode, upload to YT
