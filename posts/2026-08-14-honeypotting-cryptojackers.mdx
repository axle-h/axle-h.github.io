...
Some less than pleasant people mined cryptocurrency on my home NAS, the same machine that hosts my home movies and photos, for 17 days before I noticed.
...
I put investigating it off for days until I finally logged in and... what. wtf is this.
...
Too many red flags, Claude convinced me it was a cryptominer.

I scanned all child processes of this compromised next server and found the RCE vector used:
...
I find it interesting that the attacker chose to hide some processes behind benign names like `.kworkerd`,
...
The vulnerable application shipped `next@15.3.0` for some unknown reason pinned 🤦.
...
Remember those first two, we'll come back to those later big time!
...
I ran every remote address through reverse DNS, Team Cymru's IP to ASN whois and the Tor exit lists, hoping for a story.
There isn't one.
...
I also checked all 99 addresses against the Tor exit lists, using CollecTor's hourly archive.
