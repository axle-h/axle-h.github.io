...
The attack arrived through a little app I wrote last year called `diffy`: <PostLink name="diffy" />.
...
It was fully compromised in hours by four different "crews", and I collected some interesting data!
...
refused to help.
I fully expected this to be a problem but, using Claude Fable 5 & Opus 5, I only came up against some minor issues:
...
I noticed something was wrong because it was 30°C in my office and the fans on my tiny NAS box were spinning all day long.
...
This example echoes the environment of the running process back in the response body, encoded in base64.
...
- **The container ran as root.** No `USER` line in the Dockerfile, and `securityContext: {}` on both the pod and the container. The canonical Next.js standalone Dockerfile creates a `nextjs` user and I'd skipped it. `runAsNonRoot` and `readOnlyRootFilesystem` would both have made dropping a binary and running it a lot harder than it was.
...
It's like the movie [Inception](https://www.imdb.com/title/tt1375666/):
...
* The attacker only gains access to the clean application container, they wouldn't know that it is being monitored man-in-the-middle style by a VM.
...
Looking at the exploit logs I think that means that their payload crashed three times during the night and was
...
However a misconfiguration meant I did not collect any dumps except the manual ones in the morning.
It's a massive shame because a RAM dump taken any time between 03:02 and 03:17 would have caught the miner live, and might have had the wallet ID in it.
...
It was still silently being injected into every page when I tore the honeypot down the next morning!
...
> The `skl-4567/Jery-8372` repository might also not be there by the time anyone reads this as I have obviously reported it to GitHub.
...
Geolocation gives Ukraine, Romania, Sweden, Switzerland, Russia and the US, and for half of them the operator is registered
...
* **Running apps in containers is not just about convenience:**
  Most of the exploits tried to persist with methods that failed in my container or specifically because of the Alpine base's weirdness.
* **I'm not important enough to be targeted directly:**
  Four separate groups turned up inside a day, they are running the same script against the whole internet and I was just an address in the range.
  On the flip side I think I've probably lost the diffy.ax-h.com domain to the exploit lists (I've now deleted the DNS record), hopefully not the entire domain.
* **Limiting exposure to other people's bugs:** I've decided to drop massive frameworks like Next.js from any homelab
  ...
  can't be bothered. It really doesn't give me much over a statically served SPA in exchange for the MASSIVE attack surface it exposes me to.
